...
READ
        JMP     >DREAD                            ; DE00    READ      Read a single sector
WRITE
        JMP     >DWRITE                           ; DE03    WRITE     Write a single sector
VERIFY
        JMP     >DVERIFY                          ; DE06    VERIFY    Verify last sector written
RESTORE
        JMP     >DRESTORE                         ; DE09    RESTORE   Restore head to track #0
DRIVE
        JMP     >DDRIVE                           ; DE0C    DRIVE     Select the specified drive
CHKRDY
        JMP     >DCHKRDY                          ; DE0F    CHKRDY    Check for drive ready
QUICK
        JMP     >DQUICK                           ; DE12    QUICK     Quick check for drive ready
CINIT
        JMP     >DCINIT                           ; DE15    CINIT     Driver initialize (cold start)
WARM
        JMP     >DWARM                            ; DE18    WARM      Driver initialize (warm start)
SEEK
        JMP     >DSEEK                            ; DE1B    SEEK      Seek to specified track
...
VINCH
        TST     >TAPPTR                           ; allow redirection of input from terminal
...
IRQLL
        FDB     0                                 ; pointer to first entry in the table
        FCB     1

TAPPTR
        FDB     0                                 ; no terminal input redirection
