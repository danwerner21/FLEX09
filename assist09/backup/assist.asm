...
;*   Modification for n8VEM 6809 + i/o mezzanine board borutk 2013
;*  ACIA 6551 at $F004
;*  PIT  6840 at $F008
...
ACIA            EQU $ef04                         ; DEFAULT   ACIA ADDRESS
PTM             EQU $e808                         ; DEFAULT PTM ADDRESS
...
NUMVTR          EQU 27                            ; NUMBER OF VECTORS
...
BLD2
        TFR     Y,D                               ; PREPARE ADDRESS RESOLVE
...
BLD3
        LDA     ,Y+                               ; LOAD NEXT BYTE
        STA     ,X+                               ; STORE INTO POSITION
        DECB                                      ; COUNT DOWN
        BNE     BLD3                              ; LOOP UNTIL DONE
;* commented out, N8VEM has no extension rom
...
;*******************************************************
RESET
        LEAS    STACK,PCR                         ;  SETUP INITIAL STACK
        BSR     BLDVTR                            ; BUILD VECTOR TABLE
RESET2
        CLRA                                      ; ISSUE STARTUP MESSAGE
...
;************************************************
INITVT
        FDB     CMDTBL-*                          ; DEFAULT FIRST COMMAND TABLE
...
SWIVTB
        ...
        SPC     2
SWIR
        DEC     SWICNT,PCR                        ; UP "SWI" LEVEL FOR TRACE
...
;*************************************************
        SPC     1
SIGNON
        FCC     /ASSIST09/                        ; SIGNON EYE-CATCHER
        FCB     EOT
        SPC     1
ZMONTR
        STS     <RSTACK                           ; SAVE FOR BAD STACK RECOVERY
...
ZMONT2
        LDX     <VECTAB+.PTM                      ; LOAD PTM ADDRESS
...
CMDNOL
        LDX     10,S                              ; LOAD USERS PROGRAM COUNTER
...
CMDGOT
        SUBA    #CR                               ; SET ZERO IF CARRIAGE RETURN
        STA     -3,U                              ; SETUP FLAG
        LDX     <VECTAB+.CMDL1                    ; START WITH FIRST CMD LIST
CMDSCH
        LDB     ,X+                               ; LOAD ENTRY LENGTH
...
ZOUTHX
        ...
        DAA                                       ; ADJUST
SEND
        JMP     [VECTAB+.CODTA,PCR]               ; SEND TO OUT HANDLER
        SPC     1
ZOT4HS
        BSR     ZOUT2H                            ; CONVERT FIRST BYTE
ZOT2HS
        BSR     ZOUT2H                            ; CONVERT BYTE TO HEX
...
;************************************************
ZINCHP
        BSR     XQPAUS                            ; RELEASE PROCESSOR
ZINCH
        BSR     XQCIDT                            ; CALL INPUT DATA APPENDAGE
...
;************************************************
ZOTCH1
        LDA     1,S                               ; LOAD CHARACTER TO SEND
        LEAX    <ZPCRLS,PCR                       ; DEFAULT FOR LINE FEED
        CMPA    #LF                               ; ? LINE FEED
        BEQ     ZPDTLP                            ; BRANCH TO CHECK PAUSE IF SO
ZOTCH2
        BSR     SEND                              ; SEND TO OUTPUT ROUTINE
ZOTCH3
        INC     <SWICNT                           ; BUMP UP "SWI" TRACE NEST LEVEL
...
ZPCRLS
        ...
        SPC     1
ZPCRLF
        LEAX    ZPCRLS,PCR                        ; READY CR,LF STRING
...
;*************************************************
ZPDTLP
        BSR     SEND                              ; SEND CHARACTER TO DRIVER
ZPDTA1
...
;******************************************
ZPAUSE
        BSR     XQPAUS                            ; RELEASE CONTROL AT EVERY LINE
...
CHKSEC
        COMB                                      ; SET CARRY
CHKRTN
        RTS                                       ; RETURN TO CALLER WITH CC SET
CHKWT
        BSR     XQPAUS                            ; PAUSE FOR A MOMENT
...
XQPAUS
        JMP     [VECTAB+.PAUSE,PCR]               ; TO PAUSE ROUTINE
XQCIDT
        JSR     [VECTAB+.CIDTA,PCR]               ; TO INPUT ROUTINE
...
NMITRC
        LBRA    CTRCE3                            ; NO, TRACE ANOTHER INSTRUCTION
        SPC     1
REGPRS
        LBSR    REGPRT                            ; PRINT REGISTERS AS FROM COMMAND
        RTS                                       ; RETURN TO CALLER
        SPC     1
;* JUST EXECUTED THRU A BRKPNT.  NOW CONTINUE NORMALLY
NMICON
        CLR     <MISFLG                           ; CLEAR THRU FLAG
        LBSR    ARMBK2                            ; ARM BREAKPOINTS
RTI
        RTI                                       ; AND CONTINUE USERS PROGRAM
        SPC     1
...
;* OUTPUT: DPR LOADED TO WORK PAGE
        SPC     1
ERRMSG
        FCB     '?',BELL,$20,EOT                  ; ERROR RESPONSE
        SPC     1
LDDP
...
ERROR
        LEAX    ERRMSG,PCR                        ; LOAD ERROR REPORT
...
;***********************************************
ZBKPNT
        BSR     REGPRS                            ; PRINT OUT REGISTERS
ZBKCMD
        LBRA    CMDNEP                            ; NOW ENTER COMMAND HANDLER
        SPC     3
;********************************************
...
IRQR            EQU *                             ; IRQ ENTRY
RSRVDR
        BSR     LDDP                              ; SET BASE PAGE, VALIDATE STACK
...
;* ALL REGISTERS TRANSPARENT
        SPC     1
CODTA
        PSHS    U,D,CC                            ; SAVE REGISTERS,WORK BYTE
        LDU     #ACIA                             ; ADDRESS ACIA
        BSR     CODTAO                            ; CALL OUTPUT CHAR SUBROTINE
CODTRT
        PULS    PC,U,D,CC                         ; RESTORE REGISTERS AND RETURN

CODTAD
        LBSR    XQPAUS                            ; TEMPORARY GIVE UP CONTROL
CODTAO
        LDB     1,U                               ; LOAD ACIA CONTROL REGISTER
...
;* REGISTERS ARE VOLATILE
        SPC     1
BSDTA
        LDU     2,S                               ; U=TO ADDRESS OR OFFSET
        TST     6,S                               ; ? PUNCH
...
BSDSRT
        PULS    PC,X,A                            ; RETURN WITH Z SET PROPER
        SPC     1
BSDEOL
        INCA                                      ; ? VALID CHECKSUM
...
;*                  S+0=BYTE COUNT
BSDPUN
        LDU     <VECTAB+.PAD                      ; LOAD PADDING VALUES
...
;*PUNCH CHECKSUM
        ...
        SPC     1
BSPUN2
        ADDB    ,X                                ; ADD TO CHECKSUM
BSPUNC
        LBRA    ZOUT2H                            ; SEND OUT AS HEX AND RETURN
        SPC     1
BSPSTR
        FCB     'S','1',EOT CR,LF,NULLS,S,1
BSPEOF
        FCC     /S9030000FC/                      ; EOF STRING
        FCB     CR,LF,EOT
        SPC     3
...
HSHLNE
        ...
        LDB     #16                               ; NEXT SIXTEEN
HSHNXT
        SWI                                       ; CONVERT BYTE TO HEX AND SEND
        FCB     OUT2HS                            ; FUNCTION
        DECB                                      ; COUNT DOWN
...
HSHDOT
        LDA     #'.'                              ; CONVERT INVALID TO A BLANK
HSHCOK
        SWI                                       ; SEND CHARACTER
        FCB     OUTCH                             ; FUNCTION
        DECB                                      ; ? DONE
...
;        CPX     2,S                               ; ? PAST LAST ADDRESS
        FCB     $ac,$62                           ; BECAUSE LWASM IS BROKEN?
...
REGMSK
        ...
        SPC     1
REGPRT
        CLRA                                      ; SETUP PRINT ONLY FLAG
REGCHG
        LEAX    4+12,S                            ; READY STACK VALUE
        PSHS    Y,X,A                             ; SAVE ON STACK WITH OPTION
        LEAY    REGMSK,PCR                        ; LOAD REGISTER MASK
REGP1
        LDD     ,Y+                               ; LOAD NEXT CHAR OR <=0
...
REGP3
        SWI                                       ; PERFORM BYTE HEX
        FCB     OUT2HS                            ; FUNCTION
REG4
        LDD     ,Y+                               ; TO FRONT OF NEXT ENTRY
        TSTB                                      ; ? END OF ENTRIES
...
REGRTN
        ...
        SPC     1
REGCNG
        BSR     BLDNNB                            ; INPUT BINARY NUMBER
...
REGNXC
        STA     ,S                                ; SAVE DELIMITER IN OPTION
;*                     (ALWAYS > 0)
...
EXPDLM
        BSR     BLDHXI                            ; CLEAR NUMBER, CHECK FIRST CHAR
...
;****************************************
BLDHXI
        CLR     <NUMBER                           ; CLEAR NUMBER
        CLR     <NUMBER+1                         ; CLEAR NUMBER
BLDHEX
        BSR     READ                              ; GET INPUT CHARACTER
BLDHXC
        BSR     CNVHEX                            ; CONVERT AND TEST CHARACTER
        BNE     CNVRTS                            ; RETURN IF NOT A NUMBER
        LDB     #16                               ; PREPARE SHIFT
        MUL                                       ; BY  FOUR PLACES
        LDA     #4                                ; ROTATE BINARY INTO VALUE
BLDSHF
        ASLB                                      ; OBTAIN NEXT BIT
...
CNVGOT
        ANDA    #$0F                              ; CLEAR HIGH HEX
CNVOK
        ORCC    #4                                ; FORCE ZERO ON FOR VALID HEX
CNVRTS
        RTS                                       ; RETURN TO CALLER
        SPC     2
...
CGO
        BSR     GOADDR                            ; BUILD ADDRESS IF NEEDED
        RTI                                       ; START EXECUTING
        SPC     1
;* FIND OPTIONAL NEW PROGRAM COUNTER. ALSO ARM THE
;* BREAKPOINTS.
GOADDR
        PULS    Y,X                               ; RECOVER RETURN ADDRESS
...
ARMNSW
        INC     <MISFLG                           ; FLAG THRU A BREAKPOINT
...
CMEM
        LBSR    CDNUM                             ; OBTAIN ADDRESS
CMEMN
        STD     <ADDR                             ; STORE DEFAULT
CMEM2
        LDX     <ADDR                             ; LOAD POINTER
        LBSR    ZOUT2H                            ; SEND OUT HEX VALUE OF BYTE
        LDA     #'-'                              ;  LOAD DELIMITER
        SWI                                       ; SEND OUT
        FCB     OUTCH                             ; FUNCTION
CMEM4
        LBSR    BLDNNB                            ; OBTAIN NEW BYTE VALUE
...
CMENUM
        LDB     <NUMBER+1                         ; LOAD LOW BYTE VALUE
...
CMNOTB
        CMPA    #LF                               ; ? LINE FEED FOR NEXT BYTE
...
CMNOTL
        CMPA    #'^'                              ; ? UP ARROW FOR PREVIOUS BYTE
...
;* UPDATE BYTE
MUPDAT
        LDX     <ADDR                             ; LOAD NEXT BYTE POINTER
...
CDISPS
        PSHS    Y,X                               ; SETUP PARAMETERS FOR HSDATA
        CMPD    2,S                               ; ? WAS IT COUNT
        BLS     CDCNT                             ; BRANCH YES
        STD     ,S                                ; STORE HIGH ADDRESS
CDCNT
        JSR     [VECTAB+.HSDTA,PCR]               ; CALL PRINT ROUTINE
...
CLOAD
        BSR     CLVOFS                            ; CALL SETUP AND PASS CODE
        FCB     1                                 ; LOAD FUNCTION CODE FOR PACKET
        SPC     1
CLVOFS
        LEAU    [,S++]                            ; LOAD CODE IN HIGH BYTE OF U
...
CVER
        BSR     CLVOFS                            ; COMPUTE OFFSET IF ANY
        FCB     -1                                ; VERIFY FNCTN CODE FOR PACKET
        SPC     3
;*******************TRACE - TRACE INSTRUCTIONS
;******************* . - SINGLE STEP TRACE
CTRACE
        BSR     CDNUM                             ; OBTAIN TRACE COUNT
        STD     <TRACEC                           ; STORE COUNT
CDOT
        LEAS    2,S                               ; RID COMMAND RETURN FROM STACK
CTRCE3
        LDU     [10,S]                            ; LOAD OPCODE TO EXECUTE
...
;******************                  BRANCH OFFSETS
COFFS
        BSR     CDNUM                             ; OBTAIN INSTRUCTION ADDRESS
...
;*************
CBKPT
        BEQ     CBKDSP                            ; BRANCH DISPLAY OF JUST 'B'
...
CBKERR
        LBRA    CMDBAD                            ; RETURN TO ERROR PRODUCE
CBKADT
        STX     ,Y                                ; ADD THIS ENTRY
...
CBKSET
        LDX     <NUMBER                           ; LOAD VALUE DESIRED
CBKLDR
        LEAY    BKPTBL,PCR                        ; LOAD START OF TABLE
...
CEND1
        LEAX    <CONV2,PCR                        ; POINT AT TABLE 2
...
;****************************************************
RSRVD
        JMP     [VECTAB+.RSVD,PCR]                ; RESERVED VECTOR
SWI3
        JMP     [VECTAB+.SWI3,PCR]                ; SWI3 VECTOR
SWI2
        JMP     [VECTAB+.SWI2,PCR]                ; SWI2 VECTOR
FIRQ
        JMP     [VECTAB+.FIRQ,PCR]                ; FIRQ VECTOR
IRQ
        JMP     [VECTAB+.IRQ,PCR]                 ; IRQ VECTOR
SWI
        JMP     [VECTAB+.SWI,PCR]                 ; SWI VECTOR
NMI
        JMP     [VECTAB+.NMI,PCR]                 ; NMI VECTOR
        SPC     3
;******************************************************
